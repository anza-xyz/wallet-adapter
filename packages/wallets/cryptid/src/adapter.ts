...
interface CryptidWallet {
    postMessage(...args: unknown[]): unknown;
}

interface CryptidWindow extends Window {
    cryptid?: CryptidWallet;
}

declare const window: CryptidWindow;

export interface CryptidWalletAdapterConfig {
    network?: WalletAdapterNetwork;
    pollInterval?: number;
    pollCount?: number;
}

export class CryptidWalletAdapter extends BaseMessageSignerWalletAdapter {
    private _network: WalletAdapterNetwork;
    private _connecting: boolean;
    private _wallet: Wallet | null;

    constructor(config: CryptidWalletAdapterConfig = {}) {
        super();
    ...
    get ready(): boolean {
        return true;
        // typeof window !== 'undefined' && typeof window.cryptid?.postMessage === 'function';
    }
    ...
    async connect(): Promise<void> {
        try {
            ...
            let wallet: Wallet;
            try {
                wallet = new Wallet(this._network);
