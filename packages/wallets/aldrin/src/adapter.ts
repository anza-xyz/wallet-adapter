...
interface AldrinWallet {
    postMessage(...args: unknown[]): unknown;
}

interface AldrinWindow extends Window {
    ccai?: AldrinWallet;
}

declare const window: AldrinWindow;

export interface AldrinWalletAdapterConfig {
    provider?: string | AldrinWallet;
    network?: WalletAdapterNetwork;
    pollInterval?: number;
    pollCount?: number;
}

export class AldrinWalletAdapter extends BaseMessageSignerWalletAdapter {
    private _provider: string | AldrinWallet | undefined;
    private _network: WalletAdapterNetwork;
    private _connecting: boolean;
    private _wallet: Wallet | null;

    constructor(config: AldrinWalletAdapterConfig = {}) {
        super();
        this._provider = config.provider || (typeof window === 'undefined' ? undefined : window.ccai);
    ...
    get ready(): boolean {
        return (
            ...
            (typeof window !== 'undefined' && typeof window.ccai?.postMessage === 'function')
        );
    }
    ...
    async connect(): Promise<void> {
        try {
            ...
            const provider = this._provider || (typeof window !== 'undefined' && window.ccai);
