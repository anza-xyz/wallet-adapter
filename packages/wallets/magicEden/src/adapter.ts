...
interface MagicEdenEvents {
  connect(...args: unknown[]): unknown;
  disconnect(...args: unknown[]): unknown;
  accountChanged(newPublicKey: PublicKey): unknown;
}

interface MagicEdenWallet extends EventEmitter<MagicEdenEvents> {
...
export const MagicEdenName = "MagicEden" as WalletName<"MagicEden">;
...
export class MagicEdenWalletAdapter extends BaseMessageSignerWalletAdapter {
  ...
  constructor(config: MagicEdenWalletAdapterConfig = {}) {
    ...
    if (this._readyState !== WalletReadyState.Unsupported) {
      if (isIosAndRedirectable()) {
        ...
        scopePollingDetectionStrategy(() => {
          if (window.magicEden?.solana || window.solana) {
  ...
  async connect(): Promise<void> {
    try {
      ...
      const wallet = window.magicEden?.solana || window.solana!;
