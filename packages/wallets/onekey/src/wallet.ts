import { Wallet, WalletName } from '@solana/wallet-adapter-base';
import { OneKeyWalletAdapter, OneKeyWalletAdapterConfig } from './adapter';

export const OneKeyWalletName = 'OneKey' as WalletName;

export const getOneKeyWallet = (config: OneKeyWalletAdapterConfig = {}): Wallet => ({
    name: OneKeyWalletName,
    url: 'https://onekey.so',
    icon: 'data:image/svg+xml;base64,',
    adapter: new OneKeyWalletAdapter(config),
});
