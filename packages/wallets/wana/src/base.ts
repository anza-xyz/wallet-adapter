...
interface WanaWallet {
    postMessage(...args: unknown[]): unknown;
}

interface WanaWindow extends Window {
    wana?: WanaWallet;
}

declare const window: WanaWindow;

export interface WanaWalletAdapterConfig {
    provider?: string | WanaWallet;
    network?: WalletAdapterNetwork;
    timeout?: number;
}

export abstract class BaseWanaWalletAdapter extends BaseMessageSignerWalletAdapter {
    protected _provider: string | WanaWallet | undefined;
    ...
    constructor({ provider, network = WalletAdapterNetwork.Mainnet, timeout = 10000 }: WanaWalletAdapterConfig = {}) {
        super();
        ...
        if (this._readyState !== WalletReadyState.Unsupported) {
            if (typeof this._provider === 'string') {
                ...
                scopePollingDetectionStrategy(() => {
                    if (typeof window.wana?.postMessage === 'function') {
    ...
    async connect(): Promise<void> {
        try {
            ...
            const provider = this._provider || window!.wana!;
