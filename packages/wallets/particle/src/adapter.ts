...
export class ParticleAdapter extends BaseMessageSignerWalletAdapter {
  ...
  constructor(config: ParticleAdapterConfig = {}) {
    ...
    const { config: nestedConfig = {} } = config;
    ...
    if (config.preferredAuthType) {
      if (typeof config.preferredAuthType === "object") {
        if (config.preferredAuthType.setAsDisplay) {
          (async () => {
            try {
              this.icon = await import(
                `./${config.preferredAuthType.type}.ts`
          ...
          this.name = (config.preferredAuthType.type.charAt(0).toUpperCase() +
            config.preferredAuthType.type.slice(1)) as WalletName<any>;
        }
      }
    }
  }
  ...
  async connect(): Promise<void> {
    try {
      ...
      try {
        particleNetwork = new ParticleClass(this._config?.config);
