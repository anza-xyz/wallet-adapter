...
Require `@solana/web3.js` `^1.99.0`, which widens `TransactionVersion` to include `1`, so dapps can check `adapter.supportedTransactionVersions.has(1)` in a type-checked way.
